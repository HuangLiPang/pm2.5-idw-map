#!/usr/bin/env python
"""
Making gif for pm2.5 data portal website
...
for loc in areas:
	...
	# glob.glob returns results in an arbitrary order.
	file_list = sorted(glob.glob(imgSavingPath + loc + "/*.png"))
	# strptime returns datetime.datetime(2018, 6, 12, 13, 30)
	date_list = [datetime.strptime(i, file_prefix + "%Y-%m-%d %H:%M.png") for i in file_list]
	
	# find the index to in date list insert time_lowerbound and current
	...
	# im.save( gifSavingPath + loc + " " + timestamp +'_00_last_' + str( HOURS ) + 'h.gif', save_all=True, append_images=image, duration=DURATION, loop=LOOP )
	im.save(gifSavingPath + loc + '_latest_last_' + str( HOURS ) + 'h.gif',\
					save_all=True,\
					append_images=image,\
					duration=DURATION,\
					loop=LOOP)
